jQuery(document).ready(function(){
	...
	$('.link_video a').click(function(event){
	event.preventDefault()
	var h_e = $(window).scrollTop();
	$('.body_mask').fadeIn(300);
	$('.video_popap').css('top', (h_e+50) + 'px').slideDown(300);
	});
	
	$('.body_mask, .close').click(function(){
		$('.body_mask').fadeOut(300);
		$('.form_popap, .ok_popap, .write_popap, .application_popap, .aquarium_popap, .products_popap, .video_popap').slideUp(300);
	});
