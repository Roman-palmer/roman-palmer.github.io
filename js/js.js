jQuery(document).ready(function(){
	...
	$('.span_min').click(function(){ 
		...
		$('.select_open').click(function(){
			if($(this).hasClass('active')){
				$(this).removeClass('active');
				$(this).next('.select_body').slideUp(300);
				$(this).closest('.div_select ').css('z-index','0');
			} else{
				$(this).addClass('active');
				$(this).next('.select_body').slideDown(300);
				$(this).closest('.div_select ').css('z-index','2');
			}
				});
			$('.select_body li').click(function(){
				var  select_body = $(this).html();	
				$(this).closest('.div_select').find('.select_open > div').html(select_body);
